...
Отброшено 37,334 строк, после чтения файла
...
Тип сканирования Bitmap Index Scan. Bitmap Index Scan - сканирование GIN-индекса idx_предмет_поиск, в ходе которого по триграммам слова "Математика" ищутся все строки, содержащие нужные последовательности символов, создаётся bitmap - структура, где каждый бит соответствует странице таблицы и указывает, есть ли на ней потенциально подходящие строки, операция выполняется чрезвычайно быстро (за 0.001 мс) и использует всего 25 буферов в памяти
