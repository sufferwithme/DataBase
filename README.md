...
При выполнении запроса происходит последовательное сканирование всей таблицы (Seq Scan), что занимает 14.440 мс даже для относительно небольшой базы данных (относительно большое значение). Без специализированных индексов такие запросы всегда будут требовать полного сканирования таблицы, что неприемлемо для систем с растущим объемом данных и требованиями к отзывчивости
...
Поэтому, оптимизируем запросы при помощи GIN-индекса, который позволяет эффективно выполнять поиск по шаблонам с начальными и конечными символами подстановки (LIKE '%текст%')
...
После оптимизации мы видим, что производительность запроса значительно улучшилась: время выполнения сократилось с 14.440 мс до 5.957 мс, что демонстрирует эффективность использования GIN-индекса. Хотя общее количество обработанных буферов увеличилось с 298 до 323, это увеличение компенсируется кардинальным изменением метода доступа к данным — вместо полного последовательного сканирования таблицы теперь используется эффективный индексный поиск, что обеспечивает лучшее масштабирование при росте объема данных и позволяет избежать линейного увеличения времени выполнения запроса
